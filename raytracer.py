...
LIGHT_AMBIENT = np.array((0.2, 0.2, 0.2))
F_CONST = 24
...
class Sphere:
	...
	def intersection(self, ray):
		...
		p = 2 * np.dot(ray.dir, ray.r0 - self.centre)
		q = norm(ray.r0 - self.centre)**2 - self.radius**2
		delta = p**2 - 4 * q
		if delta > 0:
			t1 = (-q + np.sqrt(delta)) / 2
			t2 = (-q - np.sqrt(delta)) / 2
...
class RayTracedImage:
	...
	def __init__(self, xmin, xmax, ymin, ymax, numdiv, eyedist, background, objects):
		...
		self.lightpos = np.array((0, ymax+10, 0))
	...
	def trace(self, ray):
		...
		if ray.hit is not None:
			...
			shadow_ray = Ray(ray.hit, self.lightpos - ray.hit)
			shadow_ray.closest_intersection(self.objects)
			obj_normal = normalize(ray.hit - obj.centre)
			too_light = normalize(self.lightpos - ray.hit)
			if shadow_ray.hit is not None:
				if shadow_ray.dist < distance(self.lightpos, ray.hit):
					color_out = LIGHT_AMBIENT * obj.color
				else:
					color_out = LIGHT_AMBIENT * obj.color + np.dot(too_light, obj_normal) * obj.color 
			else:
				color_out = LIGHT_AMBIENT * obj.color + np.dot(too_light, obj_normal) * obj.color 
			color_out = np.abs(color_out)
		return color_out
	...
	def genourate(self):
		...
		for i in range(self.numdiv):
			...
			for j in range(self.numdiv):
				...
				ray.closest_intersection(objects)
				if ray.index is not None:
					self.img_grid[i][j] = objects[ray.index].color
				else:
					self.img_grid[i][j] = self.background
				if ray.hit is not None:
					print(f"image[{i}][{j}]={objects[ray.index].color}")
				
		plt.imsave("img.png", self.img_grid)


s1 = Sphere((0, -10, -10), 5, (1, 0, 0)) 
img = RayTracedImage(-100, 100, -200, 200, 100, 1, (0.5, 0.5, 0.5), [s1])
img.genourate()
